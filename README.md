...
`orc` is a planned orchestrator for coding agents.
...
`go build ./cmd/orc` today produces a program that prints its version and nothing else.
...
- **Headless workers.** Each task attempt runs one coding agent in its own worktree with its own `HOME`, and `orc` reads the agent's structured event stream instead of its screen.
...
Every `orc` command it names is planned and does not exist yet.

## Building

`orc` is written in Go 1.27 with no dependencies outside the standard library.

```sh
go vet ./...
go test ./...
go build -trimpath ./cmd/orc
```
